...
check_dice([A | L]) :- check_dice_aux([A | L], A).

% First argument is the first dice of the cycle
check_dice_aux([Last], Fst) :- beats(Last, Fst).
check_dice_aux([A1 | [A2 | L]], Fst) :-
   beats(A1, A2),
   check_dice_aux([A2 | L], Fst).
...
a_wins_b_wins([[X, Y] | Comb], Awins, Bwins) :-
   ...
   !.
a_wins_b_wins([[X, X] | Comb], Awins, Bwins) :-
   a_wins_b_wins(Comb, Awins, Bwins),
   !.
